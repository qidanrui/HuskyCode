...
class sentenceObject : Husky::BaseObject{
  int label;
...
class word2vec {
private:
  int MAX_STRING, EXP_TABLE_SIZE, MAX_EXP, MAX_SENTENCE_LENGTH, MAX_CODE_LENGTH;
  int vocab_hash_size;
  char *train_file, *output_file;  
  char *save_vocab_file, *read_vocab_file;  
  struct vocab_word *vocab;  //对于语料库中的每一个词放一个词的struct
  int binary, cbow, debug_mode, window, min_count, min_reduce;  
  int *vocab_hash;  
  long long vocab_max_size, vocab_size, layer1_size;  
  long long train_words, word_count_actual, file_size, classes;  
  double alpha, starting_alpha, sample;  
...
public:
  word2vec(){
    ...
    train_file = (char *)malloc(MAX_STRING * sizeof(char));
    output_file= (char *)malloc(MAX_STRING * sizeof(char));  
    save_vocab_file = (char *)malloc(MAX_STRING * sizeof(char));
    read_vocab_file = (char *)malloc(MAX_STRING * sizeof(char)); 
...
// 从文件流中读取一个词，并返回这个词在词汇表中的位置  
  ...
  {  
    ...
    //while (1)  
    //{  
    //create vocab list from train file
    ...
    auto parse_wc = [&] (boost::string_ref & chunk) {
        ...
        for (auto &word : tok){
          int i = SearchVocab(word);//返回该词在词汇表中的位置  
          if (i == -1)//该词之前不存在  
          {  
            int a = AddWordToVocab(word);//把该词添加到词汇表中  
            vocab[a].cn = 1;  
          }  
  ...
  {  
    ...
    auto parse_wc = [&] (boost::string_ref & chunk) {
      ...
      for (auto &w : tok) {
          ...
          auto it = tok2.begin();
          a = AddWordToVocab(*it);//把该词添加到词汇中，并返回该词的位置 
          vocab[a].cn = *it++;
      }
    };
    #ifdef WITH_HDFS8
  ...
  void TrainModelDistributed(std::vector<std::string> sentence)  
  {  
    ...
    {  
      ...
      while (1)  
      {  
        word = SearchVocab(sentence[sentence_length]);//从文件流中读取一个词，并返回这个词在词汇表中的位置  
        //if (feof(fi)) break;  
        if (word == -1) continue;  
        word_count++;  
        if (word == 0) break;  
  ...
  {  
    ...
    if (read_vocab_file[0] != 0)  
      ...
      LearnVocabFromTrainFile();//从训练文件学习词汇表  
    ...
    //将输入文件分成一个一个sentence存入对象并分布式执行
    Husky::BaseWorker & worker = Husky::Context::get_worker<Husky::BaseWorker>();
    auto & sentenceList = worker.create_list<sentenceObject>("sentence_list");

    auto parse_wc = [&] (boost::string_ref & chunk) {
        if (chunk.size() == 0) return;
        boost::char_separator<char> sep(" \t");
        boost::tokenizer<boost::char_separator<char>> tok(chunk, sep);
        std::vector<std::string> tempSentenceList;
        int len = 0;
        for (auto &word : tok){
          tempSentenceList.push_back(word);
          len++;
        }
        if (len <= MAX_SENTENCE_LENGTH) worker.add_object(sentenceList, tempSentenceObject);
        else {
          int pos = 0;
          while (pos < len){
            sentenceObject tempSentenceObject;
            for (int i = pos; i < min(len, pos + MAX_SENTENCE_LENGTH); i++){
              tempSentenceObject.content.push_back(tempSentenceList[i]);
            }
            pos = i + 1;
            worker.add_object(sentenceList, tempSentenceObject);
          }
        }
    };
    #ifdef WITH_HDFS8
    ...
    infmt.set_input(Husky::Context::get_params("input"));
    worker.load(infmt, parse_wc);
    worker.list_execute(sentenceList, [&](sentenceObject &tmpObj){
      TrainModelDistributed(tmpObj.content);
    });

    //output，将得到的结果直接输出或者进行kmeans聚类
    //fo = fopen(output_file, "wb");  
    if (classes == 0) //不需要聚类，只需要输出词向量  
    {  
      // Save the word vectors  
      //fprintf(fo, "%lld %lld\n", vocab_size, layer1_size); 
      Husky::log_msg("vocab_size:" + std::to_string(vocab_size) + "layer1_size:" + std::to_string(layer1_size));  
      for (a = 0; a < vocab_size; a++)  
      {  
        //fprintf(fo, "%s ", vocab[a].word);  
        std::string log = vocab[a].word + " : [";
        if (binary)  
          for (b = 0; b < layer1_size; b++)  
              //fwrite(&syn0[a * layer1_size + b], sizeof(real), 1, fo); 
            log += std::to_string(syn0[a * layer1_size + b]); 
        else  
          for (b = 0; b < layer1_size; b++)  
    ...
    {  
      ...
      double closev, x;  
      double *cent = (real *)calloc(classes * layer1_size, sizeof(real));//质心数组  
      for (a = 0; a < vocab_size; a++)  
        cl[a] = a % clcn;//任意分类？  
      for (a = 0; a < iter; a++)  
      {  
      ...
      for (a = 0; a < vocab_size; a++)  
        //fprintf(fo, "%s %d\n", vocab[a].word, cl[a]);  
        Husky::log_msg(vocab[a].word + std::to_string(cl[a]));
      free(centcn);  
      free(cent);  
      free(cl);  
    }  
...
void runWorker(){
  Husky::BaseWorker & worker = Husky::Context::get_worker<Husky::BaseWorker>();
  auto & tempVocabList = worker.create_list<SWRObject>("temp_vocab_list");

  word2vec Word2Vec;
  Word2Vec.runWord2Vec();
